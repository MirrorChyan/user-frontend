...
export default function YearMonthPicker({ onChange, initialYearMonth, showArrow }: PropsType) {
  ...
  const allYears = Array(currentYear - START_YEAR + 1).fill(0).map((_, i) => StrYear(START_YEAR + i));
  const allMonths = Array(12).fill(0).map((_, i) => StrMonth(i + 1));
  ...
  const isMaxMonth = selectedYear === StrYear(currentYear) && selectedMonth === StrMonth(currentMonth);
    ...
    <div className="flex w-full items-center overflow-hidden rounded-2xl">
      {showArrow && (
        <button
          onClick={goPrevMonth}
          disabled={isMinMonth}
          className={`mx-2 p-2 rounded-full transition-colors border border-transparent hover:bg-neutral-100 dark:hover:bg-neutral-800 disabled:opacity-40 disabled:cursor-not-allowed`}
          aria-label="<"
        >
      ...
      <div className="flex w-full items-center overflow-hidden rounded-2xl">
        ...
        >
          {allYears.map((year) => (
            <SelectItem key={year} value={year} textValue={year}>{year}</SelectItem>
          ))}
        </Select>
        <Select
        ...
        >
          {getMonthsForYear(selectedYear).map((month) => (
            <SelectItem key={month} value={month} textValue={month}>{month}</SelectItem>
          ))}
        </Select></div>
      {showArrow && (
        <button
