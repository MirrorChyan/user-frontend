...
import HtmlFormPayModal from "@/components/checkout/HtmlFormPayModal";
...
export default function Checkout(params: CheckoutProps) {
    ...
    (async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setPaymentMethod(detail.alipay_id ? "alipay" : "wechatPay");
          setPlanInfo(detail);
        }
      } catch (error) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (jresp.code != 0) {
        ...
        const resp_bak = await fetch(
          `${CLIENT_BACKEND}/api/billing/order/${platform}/create?${params}`
        );
  ...
  const finalPrice = (
    +((discount ? planInfo?.price : planInfo?.checkout_price) ?? 0) * params.rate
  ).toFixed(2);
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600 dark:text-gray-400">
                      {t(discount ? "originalPrice" : "Price")}
                    </span>
                    <span
                      className={
                        discount
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  {locale !== "zh" && planInfo?.afdian_info && (
                    <PaymentOption
                      checked={paymentMethod === "afdian"}
                      onClick={() => handlePaymentMethodChange("afdian")}
                      name={t("afdian")}
                    >
                      <div className="0 mr-3 flex h-10 w-10 items-center justify-center rounded-lg">
                        <svg
                          className="icon"
